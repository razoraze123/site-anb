...
export async function GET(context) {
  try {
    ...
    const evenementsRes = await db.prepare(
      `SELECT id, title, date, event_date, place, category, max_places, status, inscriptions_ouvertes, bg_gradient, created_at,
        (SELECT COUNT(*) FROM inscriptions i WHERE i.event_id = evenements.id) AS registered_count
       FROM evenements ORDER BY event_date IS NULL, event_date ASC`
    ).all();
